...
class InsectSpecie(models.Model):
    ...
    name = fields.Char('Insect Binomial Name', compute='_compute_name', store=True)
    specie = fields.Char('Insect Specie Name', default='', required=True)
    ...
    be_iucn_status_id = fields.Many2one('insect.eu.iucn.status', string='BE IUCN status')
    introduced_id = fields.Many2one('insect.introduced', string='Introduced')
    comment = fields.Char('Comment')

    @api.depends('specie', 'genus_id.name')
    def _compute_name(self):
        for insect in self:
            insect.name = '%s %s' % (insect.specie, insect.genus_id.name)